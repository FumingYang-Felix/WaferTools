#!/usr/bin/env python3
"""
image_transform.py - image resize and affine transformation tool

features:
1. resize input image
2. affine transformation: scale -> translation -> rotation
   (translation parameters will be adjusted automatically based on resize ratio)
...
def resize_image(img, scale_x, scale_y):
    """resize image (using 0-1 ratio)"""
    if scale_x is None and scale_y is None:
        return img
    
    h, w = img.shape[:2]
    
    # if only one ratio is specified, keep aspect ratio
    if scale_x is None:
        scale_x = scale_y
    ...
    print(f"resize ratio: {scale_x:.2f}x{scale_y:.2f}")
    print(f"resize size: {resized.shape}")
    return resized
...
def main():
    parser = argparse.ArgumentParser(description='image resize and affine transformation tool')
    ...
    parser.add_argument('--resize', nargs=2, type=float, metavar=('SCALE_X', 'SCALE_Y'), 
                       help='resize ratio (0-1, scale_x scale_y)')
    parser.add_argument('--scale', nargs=2, type=float, metavar=('SCALE_X', 'SCALE_Y'), 
                       default=[1.0, 1.0], help='scale ratio (default: 1.0 1.0)')
    parser.add_argument('--dx', type=float, default=0, help='X direction translation pixel (based on resized image, default: 0)')
    parser.add_argument('--dy', type=float, default=0, help='Y direction translation pixel (based on resized image, default: 0)')
    ...
    try:
        ...
        # create affine transformation matrix
        # note: dx and dy parameters are now based on resized image size
        ...
        if args.resize:
            print(f"  resize ratio: {args.resize[0]:.2f}x{args.resize[1]:.2f}")
        print(f"  scale: {args.scale[0]}x{args.scale[1]}")
        print(f"  translation: dx={args.dx}, dy={args.dy} (based on resized image)")
